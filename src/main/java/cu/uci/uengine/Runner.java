...
public class Runner {
    ...
    private void inFilesPreprocessor(SubmissionJudge submit, File[] inDataFiles) throws IOException {
        switch (submit.getLanguage().getName()) {
            case "Prolog":
                for (int i = 0; i < inDataFiles.length; i++) {
                    ...
                    File newFile = new File(submit.getTmpDirSid().getAbsolutePath(), file.getName());
